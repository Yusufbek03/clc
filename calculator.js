...
class CreditCalculator {
        ...
        if (paymentType === 'annuity') {
            ...
            for (let month = 1; month <= termInMonths; month++) {
                ...
                if (annuityLimit && month === termInMonths && remainingBalance > 0) {
                    const lastPayment = Math.min(monthlyPayment, remainingBalance + interestPayment);
                ...
                const inflationAdjustedPayment = (monthlyPayment + additionalPayment) / inflationFactor;
                ...
                totalPayments += monthlyPayment + additionalPayment;
    ...
    updateCharts(results) {
        ...
        console.log('- loanTerm:', results.loanTerm);
    ...
    exportToExcel() {
        try {
            ...
            // 2. Лист с результатами
            const resultsData = [
                ['РЕЗУЛЬТАТЫ РАСЧЁТА'],
                [''],
                ['Параметр', 'Значение'],
                ['Сумма кредита', `${formatCurrency(results.loanAmount)} ₽`],
                ['Срок кредита', `${results.termInMonths} месяцев`],
                ['Процентная ставка', `${(formData.monthlyRate * 12 * 100).toFixed(2)}% годовых`],
                ['Тип платежей', formData.paymentType === 'annuity' ? 'Аннуитетные' : 'Дифференцированные'],
                [''],
            ...
            if (formData.considerInflation && results.inflationAdjustedOverpayment !== null) {
                resultsData.push(['Переплата с учётом инфляции', `${formatCurrency(results.inflationAdjustedOverpayment)} ₽ (${results.inflationAdjustedOverpaymentPercentage.toFixed(2)}%)`]);
                resultsData.push(['Общая сумма с учётом инфляции', `${formatCurrency(results.totalInflationAdjustedPayments)} ₽`]);
            }
            
            // Добавляем информацию о комиссиях, если они есть
            if (results.totalCommission > 0) {
                resultsData.push(['']);
                resultsData.push(['Комиссии', `${formatCurrency(results.totalCommission)} ₽`]);
            }
            
            const resultsSheet = XLSX.utils.aoa_to_sheet(resultsData);
            
            // Форматирование листа результатов
            const resultsRange = XLSX.utils.decode_range(resultsSheet['!ref']);
            
            // Форматирование заголовка
            const titleCell = XLSX.utils.encode_cell({ r: 0, c: 0 });
            if (resultsSheet[titleCell]) {
                resultsSheet[titleCell].s = {
            ...
            for (let row = 2; row <= resultsRange.e.r; row++) {
                const paramCell = XLSX.utils.encode_cell({ r: row, c: 0 });
                const valueCell = XLSX.utils.encode_cell({ r: row, c: 1 });
                
                if (resultsSheet[paramCell]) {
                    resultsSheet[paramCell].s = {
                        font: { bold: true },
                        fill: { fgColor: { rgb: "F0F8FF" } }
                    };
                }
                
                if (resultsSheet[valueCell]) {
                    resultsSheet[valueCell].s = {
                        alignment: { horizontal: "right" }
                    };
                }
            }
            
            // 3. Лист с графиком платежей
            const scheduleHeaders = ['№', 'Дата', 'Платеж (₽)', 'Основной долг (₽)', 'Проценты (₽)', 'Остаток (₽)', 'Описание'];
            ...
            results.schedule.forEach((payment, index) => {
                ...
                if (formData.considerInflation && payment.inflationAdjustedPayment) {
                    row.push(Number(payment.inflationAdjustedPayment) || 0, Number(payment.inflationAdjustedBalance) || 0);
                }
            ...
            if (formData.considerInflation && results.totalInflationAdjustedPayments) {
                totalRow.push(results.totalInflationAdjustedPayments, 0);
            }
            ...
            const range = XLSX.utils.decode_range(scheduleSheet['!ref']);
            ...
            const numericColumns = [2, 3, 4, 5]; // Платеж, Основной долг, Проценты, Остаток
            for (let row = 1; row <= range.e.r; row++) {
                numericColumns.forEach(col => {
                    const cellAddress = XLSX.utils.encode_cell({ r: row, c: col });
                    if (scheduleSheet[cellAddress]) {
                        scheduleSheet[cellAddress].s = {
                            numFmt: '#,##0.00 ₽',
                            alignment: { horizontal: "right" }
                        };
            ...
            if (formData.considerInflation) {
                const inflationColumns = [7, 8]; // Платеж с инфляцией, Остаток с инфляцией (с учетом новой колонки Описание)
                for (let row = 1; row <= range.e.r; row++) {
                    inflationColumns.forEach(col => {
                        const cellAddress = XLSX.utils.encode_cell({ r: row, c: col });
                        if (scheduleSheet[cellAddress] && scheduleSheet[cellAddress].v !== '') {
                            scheduleSheet[cellAddress].s = {
                                numFmt: '#,##0.00 ₽',
                                alignment: { horizontal: "right" },
                                fill: { fgColor: { rgb: "F0F8FF" } }
            ...
            for (let col = 0; col <= range.e.c; col++) {
                ...
                if (scheduleSheet[cellAddress]) {
                    ...
                    if (col >= 2 && col <= 5) {
                        scheduleSheet[cellAddress].s.numFmt = '#,##0.00 ₽';
                        scheduleSheet[cellAddress].s.alignment.horizontal = "right";
                    }
                }
            }
            ...
            scheduleSheet['!cols'] = [
                ...
                { width: 12 },  // Дата
                { width: 15 },  // Платеж
                { width: 15 },  // Основной долг
                { width: 15 },  // Проценты
                { width: 15 },  // Остаток
                { width: 40 },  // Описание
                ...(formData.considerInflation ? [{ width: 20 }, { width: 20 }] : []) // Колонки инфляции
            ];
            ...
            console.log('Schedule sheet data range:', scheduleSheet['!ref']);
    ...
    exportToCSV() {
        try {
            ...
            csvContent += `Вид ставки,${formData.rateType === 'fixed' ? 'Фиксированная' : 'Плавающая'}\n\n`;
