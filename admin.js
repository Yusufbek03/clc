...
class AdminPanel {
    ...
    initializeEventListeners() {
        ...
        const testButton = document.getElementById('test-formulas');
        if (testButton) {
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`;
    ...
    // Test formulas with different variables
    testFormulas() {
        const calculators = this.config.getAllCalculators();
        const formulas = {
            title: document.getElementById('title-formula').value,
            description: document.getElementById('description-formula').value,
            h1: document.getElementById('h1-formula').value
        ...
        let testResults = '<h4 style="font-size: 13px; font-weight: 600; margin-bottom: 8px;">Результаты тестирования формул:</h4>';
        
        calculators.forEach(calculator => {
            const testTitle = this.processFormula(formulas.title, calculator);
            const testDescription = this.processFormula(formulas.description, calculator);
            const testH1 = this.processFormula(formulas.h1, calculator);
            
            testResults += `
                <div style="margin-bottom: 12px; padding: 8px; background: #f8f9fa; border-radius: 4px; border-left: 3px solid #28a745;">
                    <div style="font-weight: 600; color: #28a745; margin-bottom: 4px;">${calculator.icon} ${calculator.name}:</div>
                    <div style="font-size: 11px; margin-bottom: 2px;"><strong>Title:</strong> ${testTitle} (${testTitle.length}/60)</div>
                    <div style="font-size: 11px; margin-bottom: 2px;"><strong>Description:</strong> ${testDescription} (${testDescription.length}/160)</div>
                    <div style="font-size: 11px;"><strong>H1:</strong> ${testH1} (${testH1.length}/50)</div>
                </div>
            `;
        });
    ...
    showTestResultsModal(content) {
        // Remove existing modal
        const existingModal = document.querySelector('.test-results-modal');
        if (existingModal) {
        ...
        modal.innerHTML = `
            <div style="
                ...
                position: relative;
            ">
                <button onclick="this.closest('.test-results-modal').remove()" style="
                    position: absolute;
                    top: 10px;
                    right: 15px;
                    background: none;
